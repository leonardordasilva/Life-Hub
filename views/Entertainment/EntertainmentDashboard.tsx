                                ...
                                <div>
                                    ...
                                    <div className="max-h-[400px] overflow-y-auto custom-scrollbar space-y-3 mb-6 pr-2">
                                        {syncState.diffs.map(d => (
                                            <div key={d.id} className={`rounded-lg border transition-colors ${selectedDiffs.has(d.id) ? 'bg-pink-500/10 border-pink-500/50' : 'bg-slate-800 border-white/5'}`}>
                                                <div className="p-3 flex items-center justify-between cursor-pointer" onClick={() => toggleDiffSelection(d.id)}>
                                                    ...
                                                    <div className={`w-5 h-5 rounded border flex items-center justify-center shrink-0 ${selectedDiffs.has(d.id) ? 'bg-pink-600 border-pink-600' : 'border-slate-600'}`}>{selectedDiffs.has(d.id) && <Check className="w-3 h-3 text-white" />}</div>
                                                </div>
                                                <div className="px-3 pb-3 space-y-1.5">
                                                    {d.changes.map((c, idx) => (
                                                        <div key={idx} className="flex items-start gap-2 text-xs bg-slate-900/50 rounded-md px-2.5 py-1.5">
                                                            <span className="text-pink-400 font-semibold shrink-0 w-28">{c.label}</span>
